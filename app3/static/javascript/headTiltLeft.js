function calculateAngle(a, b, c) {
  ...
  function updateProgressBar(currentAngle) {
    ...
    if (progress === 100 && isCounting && !isReturning) {
      isReturning = true;
      messageBox.innerText = "Move your head back to center!";
      messageBox.style.display = 'block'; // Show the message box
    } else if (progress === 0 && isCounting && isReturning) {
      repsCount++;
      countView.innerText = repsCount;
      isReturning = false;
      messageBox.innerText = "Tilt your head to the left!";
      messageBox.style.display = 'block'; // Show the message box
  ...
  async function estimatePose(video, net) {
    ...
    async function detect() {
      ...
      const keypoints = pose.keypoints.reduce((acc, point) => {
        if (['nose', 'leftEar', 'leftShoulder'].includes(point.part)) {
          acc[point.part] = point.position;
        }
        return acc;
      }, {});
  
      const { nose, leftEar, leftShoulder } = keypoints;
  
      const exercise = document.getElementById('exercise-selector').value;
      let angle = 0;
  
      if (exercise === 'headTiltLeft') {
        angle = calculateAngle(nose, leftEar, leftShoulder);
      }
      ...
      ctx.arc(leftShoulder.x, leftShoulder.y, 5, 0, 2 * Math.PI);
      ctx.fill();
